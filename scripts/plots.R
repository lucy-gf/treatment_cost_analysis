...
outp_max <- max(abs(r_v_f[r_v_f$treatment_type=='outp',]$residuals))
hosp_max <- max(abs(r_v_f[r_v_f$treatment_type=='hosp',]$residuals))
rel_outp_max <- max(abs(r_v_f[r_v_f$treatment_type=='outp',]$residuals/r_v_f[r_v_f$treatment_type=='outp',]$fitted))
rel_hosp_max <- max(abs(r_v_f[r_v_f$treatment_type=='hosp',]$residuals/r_v_f[r_v_f$treatment_type=='hosp',]$fitted))

rvfp1 <- r_v_f %>%
  filter(treatment_type == 'outp') %>% 
  ggplot() +
  geom_point(aes(fitted_vals, residuals, col = study_pop),
             size = 3) +
  labs(x = "Fitted values", y = "Residual values", col = 'Age group') +
  theme_bw() +
  scale_color_manual(values = age_colors, labels = pop_labels) +
  scale_shape_manual(values = c(16,1), labels = outcome_labels) +
  facet_grid(. ~ treatment_type, scales = 'free', labeller = labeller(treatment_type = outcome_labels)) +
  # scale_x_log10() +
  # scale_y_log10() +
  scale_y_continuous(limits = c(-outp_max, outp_max)) + 
  theme(text = element_text(size = 10)); rvfp1
...
rvfp2 <- r_v_f %>%
  ...
  geom_point(aes(fitted_vals, residuals, col = study_pop),
             size = 3) +
  labs(x = "Fitted values", y = "", col = 'Age group') +
  theme_bw() +
  scale_color_manual(values = age_colors, labels = pop_labels) +
  scale_shape_manual(values = c(16,1), labels = outcome_labels) +
  facet_grid(. ~ treatment_type, scales = 'free', labeller = labeller(treatment_type = outcome_labels)) +
  # scale_x_log10() +
  # scale_y_log10() +
  scale_y_continuous(limits = c(-hosp_max, hosp_max)) + 
  theme(text = element_text(size = 10)); rvfp2

rvfp3 <- r_v_f %>%
  filter(treatment_type == 'outp') %>% 
  ggplot() +
  geom_point(aes(fitted_vals, residuals/fitted_vals, col = study_pop),
             size = 3) +
  labs(x = "Fitted values", y = "Relative residual", col = 'Age group') +
  theme_bw() +
  scale_color_manual(values = age_colors, labels = pop_labels) +
  scale_shape_manual(values = c(16,1), labels = outcome_labels) +
  facet_grid(. ~ treatment_type, scales = 'free', labeller = labeller(treatment_type = outcome_labels)) +
  # scale_x_log10() +
  # scale_y_log10() +
  scale_y_continuous(limits = c(-rel_outp_max, rel_outp_max)) + 
  theme(text = element_text(size = 10)); rvfp3
...
rvfp4 <- r_v_f %>%
  ...
  # scale_x_log10() +
  # scale_y_log10() +
  scale_y_continuous(limits = c(-rel_hosp_max, rel_hosp_max)) + 
  theme(text = element_text(size = 10)); rvfp4

rvfp1 + rvfp2 + rvfp3 + rvfp4 + plot_layout(nrow = 2,
                                            guides = 'collect')
